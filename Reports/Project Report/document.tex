...
\color{green} after \color{black} all \texttt{inter} \& \texttt{intra} specialization passes have been completed. The example \texttt{C} code is as below. It accepts \texttt{three flags} namely \texttt{-a}, \texttt{-b} \& \texttt{-c} and call a function based on the \texttt{flag} received.
...
\texttt{OCCAM} log after run was complete. We saw that there was reduction of the code size that was finally compiled to binary. 
...
\begin{table*}[]
	...
	\begin{tabular}{l|cccccc}
		...
		\begin{tabular}[c]{@{}l@{}}Total Unique Gadgets\\ (Excluding SYS \& Chain)\end{tabular} & 2693                          & 1189                                                                                             & 3258                                                                                                    & 2664                                                                                                     & 2664                                                                            & 3162                                                  \\ \hline
	\end{tabular}
	\caption{Dynamic Binary Analysis : Gadgets Count comparision for \color{blue} GNU Tree}
\end{table*}

\section{Observations \& Fails}%
\label{Tools}

For \texttt{DeepOCCAM} tool, using \texttt{bzip} \& \texttt{tree} source code files for generating the \texttt{inst2vec} embeddings worked out well in debloating \texttt{bzip} \& \texttt{tree} but it does defeat the purpose that we can't generate the embeddings in inst2vec first and then use it for feature vector creation in metadata stage, each time that we want to run \texttt{DeepOCCAM}. Using pre embedded \texttt{inst2vec} embeddings, we got too many \texttt{broken} runs. 

For some of the tools and libraries, \texttt{OCCAM} or any variant of \texttt{OCCAM} say \texttt{OCCAM-T} or \texttt{DeepOCCAM} did no significant reduction of the attack surface nor did it decrease static analysis metric counts, reason for that could failure in proper specialization of the function at the call-site or rewrite passes failing due to amalgamation pass not working correctly. The exact reason for this observation is unknown to us. 

\subsection{Insights}%
\label{Tools}

The way we encode the states heavily depends on the metrics that can directly help in reducing the final comparison metric we are using for comparison of two tools. It is not a trivial task to decide as to whether we should specialize a function at a call-site or not. We ran \texttt{OCCAM} and \texttt{DeepOCCAM} under a random specialization policy where we took the decision to specialize or not at random and found that in some cases, the \texttt{global fixed point} computation took a huge time (1~2) days to terminate. We definitely need a proper understanding of what are the effects of \texttt{specializing} at a call-site and also on what counts to choose for a good \texttt{hand crafted} heuristics based feature vector, \texttt{HF} in short.  
